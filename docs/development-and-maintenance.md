...
This checks the whole tree in one pass. It asserts that each skill has a
`SKILL.md`, that its frontmatter `name` and `description` are present and the
name matches the directory, that any `agents/openai.yaml` parses as YAML, and
that no skill directory has appeared outside `skills/`.

Manual checklist, if you want to verify a single skill by eye:
