...
This guide shows how to use Trident SecOps Skills after installation.

The prompting patterns below work identically on Gemini CLI, Claude Code, and
Codex — all three activate skills from the same `name` and `description`
fields. Only the session-management commands differ; see
[CLI usage](docs/cli-usage.md) for the per-agent detail.
...
| Claude Code | `/plugin` |
| Codex | skills load at startup; reference one with `$skill-name` |

The remaining commands in this section are Gemini CLI only.
...
On Claude Code, manage the bundle with `/plugin`. On Codex, restart after
editing skill files.
